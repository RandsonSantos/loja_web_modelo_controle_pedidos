...
def add_produto(nome, descricao, preco, ativo=1):
    conn = sqlite3.connect('db.sqlite3')
    c = conn.cursor()
    c.execute('INSERT INTO produtos (nome, descricao, preco, ativo) VALUES (?, ?, ?, ?)', (nome, descricao, preco, ativo))
    conn.commit()
    conn.close()
...
def get_produtos():
    ...
    produtos = c.fetchall()
    conn.close()
    return [{'id': produto[0], 'nome': produto[1]} for produto in produtos]
...
def get_produtos(query=None):
    ...
    c = conn.cursor()
    if query:
        c.execute('''SELECT * FROM produtos WHERE nome LIKE ? OR descricao LIKE ?''', ('%' + query + '%', '%' + query + '%'))
    else:
...
def editar_produto(produto_id):
    try:
        ...
        if request.method == 'POST':
            ...
            descricao = request.form.get('descricao')
            preco = request.form.get('preco')
            ativo = int(request.form.get('ativo'))
            obs = request.form.get('obs')

            c.execute('''UPDATE produtos
                         SET nome = ?, descricao = ?, preco = ?, ativo = ?, obs = ?
                         WHERE id = ?''', 
                      (nome, descricao, preco, ativo, obs, produto_id))
            conn.commit()
            conn.close()
        ...
        logging.error(f"Erro ao carregar página de edição do produto: {e}")
        return str(e), 500
